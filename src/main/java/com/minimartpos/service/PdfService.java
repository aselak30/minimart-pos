...
public class PdfService {
    ...
    private void addCompanyHeader(Document doc, PdfFont bold, PdfFont regular) throws IOException {
        ...
        doc.add(new Paragraph(company).setFont(bold).setFontSize(16)
                .setFontColor(PRIMARY).setTextAlignment(TextAlignment.CENTER).setMarginBottom(0));

        if (!address.isBlank()) {
            doc.add(new Paragraph(address).setFont(regular).setFontSize(10)
                    .setTextAlignment(TextAlignment.CENTER).setFontColor(ColorConstants.GRAY).setMarginBottom(0));
        }
        if (!phone.isBlank()) {
    ...
    private void addLogoIfPresent(Document doc) {
        try {
            java.net.URL logoUrl = getClass().getResource("/images/logo.png");
            if (logoUrl != null) {
                Image img = new Image(ImageDataFactory.create(logoUrl));
                img.setHorizontalAlignment(HorizontalAlignment.CENTER);
                img.setMaxWidth(80); // Adjust size as needed
                img.setMarginBottom(5);
                doc.add(img);
            }
        } catch (Exception e) {
            logger.warn("Could not load logo for PDF: {}", e.getMessage());
        }
    }
